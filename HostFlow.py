...
def ping(host):
    ...
    try:
        ...
        return True, ttl  # Returns True if ping is successful
    except subprocess.CalledProcessError:
        return False, 'Ping error'  # Returns False if ping fails

def dns_lookup(host):
    """Performs DNS resolution of the host and returns IP and reverse hostname."""
    try:
...
def open_rdp(event, host_pingando):
    ...
    if host_pingando:
        rdp_command = f'mstsc /v:{host_pingando}'
        try:
            subprocess.Popen(rdp_command)
        except Exception as e:
            messagebox.showerror("Error", f"Could not open RDP connection: {e}")
    else:
        messagebox.showwarning("Warning", "No available pinging host for RDP connection.")
...
def open_ssh(host):
    """Opens SSH connection after requesting access key."""
    ssh_key = simpledialog.askstring("Access Key", "Enter access key:")
    if ssh_key:
        cmd_command = f"start cmd /k ssh {ssh_key}@{host}"
        try:
            subprocess.Popen(cmd_command, shell=True)
        except Exception as e:
            messagebox.showerror("Error", f"Could not open SSH connection: {e}")
...
def run_ping_script():
    try:
        subprocess.Popen(['python', 'ping.py'])
    except Exception as e:
...
def update_table_visibility():
    for item in results_tree.get_children():
        ...
        if any(checkbox_vars[color].get() and color in host_info for color in checkbox_vars):
            results_tree.item(item, open=True)
        else:
